...
let vocab = { level: "all", sem: "all", struct: "all", status: "all", review: false, sort: "level", q: "", limit: 100, editing: null, material: "all", groupBy: "level", open: new Set(), view: "grid" };
  ...
  root.append(el("div", { class: "panel vocab-filter" },
    ...
    el("div", { class: "row", style: "margin-top:10px;justify-content:flex-end" },
      el("div", { class: "subtabs" },
        el("button", { class: "subtab" + (vocab.view === "grid" ? " on" : ""), onclick: () => { vocab.view = "grid"; renderFolders(); } }, iconEl("cards"), el("span", {}, " Lưới")),
        el("button", { class: "subtab" + (vocab.view === "detail" ? " on" : ""), onclick: () => { vocab.view = "detail"; renderFolders(); } }, iconEl("book"), el("span", {}, " Chi tiết"))))));
...
function renderFolders() {
  ...
  getDeck(store.getSettings().activeDeckId).then((deck) => {
    ...
    // P4 — học nhanh tất cả từ đang lọc (đổi nhãn khi lọc "đến hạn")
    if (cards.length && (vocab.q || vocab.status !== "all")) {
      const due = vocab.status === "due";
      host2.append(el("div", { class: "row", style: "margin-bottom:12px" },
        el("button", { class: "btn primary", onclick: () => studyScope(cards.map((c) => c.id), due ? "Đến hạn ôn" : "Từ đang lọc", "study") },
          iconEl(due ? "replay" : "play"), el("span", { class: "btn-tx" }, `${due ? "Ôn" : "Học"} ${cards.length} từ đang lọc`))));
    }
    const ctx = { progress: store.getProgress(), charMode: s.charMode, view: vocab.view, rate: s.speechRate };
    for (const g of groups) host2.append(folderEl(g, ctx));
  });
}
...
function folderEl(g, ctx = {}) {
  ...
  const prog = ctx.progress || store.getProgress();
  const learned = g.cards.reduce((n, c) => n + (prog[c.id] && prog[c.id].reps > 0 ? 1 : 0), 0);
  const pct = g.cards.length ? Math.round((learned / g.cards.length) * 100) : 0;
  const head = el("div", { class: "folder-head" },
    ...
    el("span", { class: "folder-prog" },
      el("span", { class: "folder-bar" }, el("span", { style: `width:${pct}%` + (pct >= 100 ? ";background:var(--ok)" : "") })),
      el("span", { class: "muted small" }, `${selN ? selN + " chọn · " : ""}thuộc ${learned}/${g.cards.length}`)),
  ...
  if (open) {
    ...
    if (ctx.view === "detail") {
      const wrap = el("div", { class: "folder-rows" });
      g.cards.slice(0, lim).forEach((c) => wrap.append(wordRow(c, ctx)));
      box.append(wrap);
    } else {
...
// Hàng chi tiết: 简(繁) · pinyin · Hán-Việt · nghĩa · nghe (P1 — xem được nghĩa, hợp mobile).
function wordRow(c, ctx = {}) {
  const on = vocabSel.has(c.id);
  const { main, sub } = displayHanzi(c, ctx.charMode);
  const toggle = () => { if (on) vocabSel.delete(c.id); else vocabSel.add(c.id); renderFolders(); renderSelBar(); };
  return el("div", { class: "wrow" + (on ? " on" : ""), onclick: toggle },
    el("span", { class: "wcb" + (on ? " on" : "") }, on ? "✓" : ""),
    el("span", { class: "wrow-zh" }, main, sub ? el("span", { class: "wrow-zh2 no-cc" }, sub) : null),
    el("span", { class: "wrow-py pinyin" }, c.pinyin || ""),
    el("span", { class: "wrow-mean muted small" }, (c.han_viet ? `[${c.han_viet}] ` : "") + (c.meaning || "")),
    el("button", { class: "btn ghost small wrow-audio", title: "Nghe", onclick: (e) => { e.stopPropagation(); speak(c.simplified, { rate: ctx.rate }); } }, iconEl("speaker")));
}
...
let transSavedFilter = "all";  // Bài đã dịch: all | graded | weak
...
// Trình dịch song ngữ chia đôi màn hình (theo khúc/chương): nguồn | bản dịch của bạn.
async function transTaskRunner(root) {
  ...
  const recOf = (src) => store.getTranslations().find((t) => t.source === src);
  const doneEl = el("span", { class: "chip" });
  ...
  root.append(list);
  refresh();
  root.append(el("p", { class: "muted small", style: "margin-top:12px" },
    "Bản dịch tự lưu vào “Bài đã dịch”. " + (backend ? "Vào Bài đã dịch → Mở lại một câu để Qwen3 chấm & sửa." : "Bật Qwen3 (Cài đặt) để được chấm điểm + sửa lỗi.")));
}

// Lưu/ xoá bản dịch 1 câu của truyện (keyed theo văn bản nguồn).
function saveStorySentence(source, user, dir, sourcePinyin) {
  if (!source) return;
  const existing = store.getTranslations().find((t) => t.source === source);
  if (!(user || "").trim()) { if (existing) store.deleteTranslation(existing.id); return; }
  const rec = existing
    ? { ...existing, user, dir }
    : { id: "tr-" + Date.now().toString(36) + Math.floor(Math.random() * 1e4).toString(36), dir, source, sourcePinyin: sourcePinyin || "", user, ref: "", refPinyin: "", grade: null, createdAt: new Date().toISOString() };
...
async function transWorkspaceBody(root, s, srcIsZh) {
  ...
  const srcPanel = el("div", { class: "panel stack" },
  ...
  const userCount = el("span", { class: "muted small" });
  const updUserCount = () => { userCount.textContent = `${countChars(userTa.value)} chữ`; };
  updUserCount();
  userTa.addEventListener("input", () => { transDraft.user = userTa.value; updUserCount(); });
  userTa.addEventListener("keydown", (e) => { if ((e.ctrlKey || e.metaKey) && e.key === "Enter") { e.preventDefault(); saveTransDraft(false); } });
  const userPanel = el("div", { class: "panel stack" },
    el("div", { class: "row spread" }, el("b", {}, "Bản dịch của bạn"), userCount),
    el("div", { class: "field" }, userTa),
    el("p", { class: "muted small", style: "margin:0" }, "Mẹo: Ctrl/⌘ + Enter để lưu nhanh."));
  root.append(el("div", { class: "trans-split2" }, srcPanel, userPanel));
...
function transSaved(root) {
  ...
  const all = store.getTranslations();
  if (!all.length) { root.append(emptyState("Chưa có bài dịch", "Dịch một câu rồi bấm Lưu.", "trans")); return; }
  const graded = (t) => t.grade && t.grade.score != null;
  const counts = { all: all.length, graded: all.filter(graded).length, weak: all.filter((t) => graded(t) && t.grade.score < 8).length };
  const chips = el("div", { class: "comm-chips", style: "margin-bottom:12px" });
  for (const [id, lbl] of [["all", `Tất cả (${counts.all})`], ["graded", `Đã chấm (${counts.graded})`], ["weak", `Cần ôn · điểm < 8 (${counts.weak})`]])
    chips.append(commChip(lbl, transSavedFilter === id, () => { transSavedFilter = id; renderTrans(); }));
  root.append(chips);
  const list = transSavedFilter === "graded" ? all.filter(graded) : transSavedFilter === "weak" ? all.filter((t) => graded(t) && t.grade.score < 8) : all;
  if (!list.length) { root.append(el("p", { class: "muted center", style: "padding:24px" }, "Không có bài nào trong mục này.")); return; }
  const s = store.getSettings();
  for (const t of list) {
...
let tradFolderView = "grid";     // Thư mục phồn thể: grid | detail
let tradQuizDir = "t2s";         // Quiz: t2s = 繁→简 · s2t = 简→繁
...
export async function renderTradFolder() {
  ...
  root.append(el("div", { class: "row", style: "justify-content:flex-end;margin-bottom:10px" },
    el("div", { class: "subtabs" },
      el("button", { class: "subtab" + (tradFolderView === "grid" ? " on" : ""), onclick: () => { tradFolderView = "grid"; renderTradFolder(); } }, iconEl("cards"), el("span", {}, " Lưới")),
      el("button", { class: "subtab" + (tradFolderView === "detail" ? " on" : ""), onclick: () => { tradFolderView = "detail"; renderTradFolder(); } }, iconEl("book"), el("span", {}, " Chi tiết")))));
  const tprog = store.getTradSrs();
  ...
  for (const lv of Object.keys(byLevel).map(Number).sort((a, b) => a - b)) {
    ...
    const learned = arr.reduce((n, p) => n + (tprog[p.simp] && tprog[p.simp].reps > 0 ? 1 : 0), 0);
    const pct = arr.length ? Math.round((learned / arr.length) * 100) : 0;
    const head = el("div", { class: "row spread" },
      el("b", {}, `HSK${lv}`, el("span", { class: "muted small" }, ` · ${arr.length} chữ`)),
      el("div", { class: "row" },
        el("span", { class: "folder-prog" }, el("span", { class: "folder-bar" }, el("span", { style: `width:${pct}%` + (pct >= 100 ? ";background:var(--ok)" : "") })), el("span", { class: "muted small" }, `thuộc ${learned}/${arr.length}`)),
    ...
    const toggle = (p) => { tradSel.has(p.simp) ? tradSel.delete(p.simp) : tradSel.add(p.simp); renderTradFolder(); };
    let body;
    if (tradFolderView === "detail") {
      body = el("div", { class: "folder-rows" });
      for (const p of arr) {
        const on = tradSel.has(p.simp);
        body.append(el("div", { class: "wrow" + (on ? " on" : ""), onclick: () => toggle(p) },
          el("span", { class: "wcb" + (on ? " on" : "") }, on ? "✓" : ""),
          el("span", { class: "wrow-zh trad no-cc" }, p.trad),
          el("span", { class: "wrow-py" }, p.simp + (p.pinyin ? " · " + p.pinyin : "")),
          el("span", { class: "wrow-mean muted small" }, (p.han_viet ? `[${p.han_viet}] ` : "") + (p.meaning || "")),
          el("button", { class: "btn ghost small wrow-audio", title: "Nghe", onclick: (e) => { e.stopPropagation(); speak(p.simp, { rate: s.speechRate }); } }, iconEl("speaker"))));
      }
    } else {
      body = el("div", { class: "comm-chips" });
      for (const p of arr) body.append(commChip(`${p.simp}→${p.trad}`, tradSel.has(p.simp), () => toggle(p)));
    }
    root.append(el("div", { class: "panel stack" }, head, body));
  }
}
...
async function tradQuizPanel() {
  const s = store.getSettings();
  const pairs = scopedPairs(await tradPairs());
  const s2t = tradQuizDir === "s2t";
  const box = el("div", { class: "panel stack" });
  box.append(el("div", { class: "row spread" }, el("b", {}, s2t ? "Quiz: đọc giản thể → chọn phồn thể" : "Quiz: đọc phồn thể → chọn giản thể"), tradQuiz ? el("span", { class: "muted small" }, `Điểm ${tradQuiz.score}/${tradQuiz.total}`) : null));
  // chiều quiz
  const dirRow = el("div", { class: "comm-chips" });
  dirRow.append(commChip("繁 → 简", !s2t, () => { tradQuizDir = "t2s"; tradQuiz = null; renderTradComp(); }));
  dirRow.append(commChip("简 → 繁", s2t, () => { tradQuizDir = "s2t"; tradQuiz = null; renderTradComp(); }));
  box.append(dirRow);
  if (pairs.length < 4) { box.append(el("p", { class: "muted small" }, "Phạm vi cần ≥4 chữ để làm quiz.")); return box; }
  if (tradQuiz && (tradQuiz.scopeLabel !== scopeLabel() || tradQuiz.dir !== tradQuizDir)) tradQuiz = null;
  if (!tradQuiz || tradQuiz.next) tradQuiz = makeTradQuestion(pairs, tradQuiz);
  const q = tradQuiz;
  box.append(el("div", { class: "row center", style: "gap:10px" },
    el("div", { class: "trad-quiz-q no-cc" }, q.prompt),
    el("button", { class: "btn ghost small", title: "Nghe", onclick: () => speak(q.pair.simp, { rate: s.speechRate }) }, iconEl("speaker"))));
  box.append(el("div", { class: "muted small center" }, s2t ? "Chữ giản thể trên ứng với chữ phồn thể nào?" : "Chữ phồn thể trên ứng với chữ giản thể nào?"));
  const opts = el("div", { class: "quiz-options" });
  for (const o of q.options) {
    let cls = "btn quiz-opt no-cc";
    if (q.picked) { if (o === q.answer) cls += " correct"; else if (o === q.picked) cls += " wrong"; }
...
function makeTradQuestion(pairs, prev) {
  ...
  const s2t = tradQuizDir === "s2t";
  const field = s2t ? "trad" : "simp";
  const prompt = s2t ? pair.simp : pair.trad;
  const answer = pair[field];
  const opts = new Set([answer]);
  while (opts.size < 4 && opts.size < pairs.length) opts.add(pairs[Math.floor(Math.random() * pairs.length)][field]);
  return { pair, prompt, answer, options: [...opts].sort(() => Math.random() - 0.5), picked: null, score: prev ? prev.score : 0, total: prev ? prev.total : 0, next: false, scopeLabel: scopeLabel(), dir: tradQuizDir };
}
function pickTrad(o) {
  if (tradQuiz.picked) return;
  tradQuiz.picked = o;
  tradQuiz.total++;
  if (o === tradQuiz.answer) tradQuiz.score++;
...
export async function renderWordsets() {
  ...
  if (!sets.length) {
    root.append(emptyState("Chưa có bộ từ nào", "Vào Thư mục, tick chọn từ rồi “Lưu thành bộ” để học theo nhóm của bạn.", "cards", { label: "Mở Thư mục", icon: "folder", onClick: () => navigate("vocab") }));
    return;
  }
  const deck = await getDeck(store.getSettings().activeDeckId);
